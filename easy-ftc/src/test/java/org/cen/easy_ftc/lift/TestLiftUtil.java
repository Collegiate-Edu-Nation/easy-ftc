...
public class TestLiftUtil {
    ...
    public void map_isCorrect() {
        ...
        for(int i = 0; i < controllerValues.length / 2; i++) {
            assertEquals(expectedValues[i], LiftUtil.map(controllerValues[i], deadZone), 0.01);
        }
    ...
    public void whenOneMotor_scaleDirections_isCorrect() {
        ...
        // Test power zero
        for(int i = 0; i < 3; i++) {
            double [] movements = LiftUtil.scaleDirections(powers[0], motorDirections[i]);
            for(int j = 0; j < 1; j++) {
        ...
        // Test power half
        for(int i = 0; i < 3; i++) {
            double [] movements = LiftUtil.scaleDirections(powers[1], motorDirections[i]);
            for(int j = 0; j < 1; j++) {
        ...
        // Test power full
        for(int i = 0; i < 3; i++) {
            double [] movements = LiftUtil.scaleDirections(powers[2], motorDirections[i]);
            for(int j = 0; j < 1; j++) {
